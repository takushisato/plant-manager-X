...
class BreakSetting(BaseModel):
    ...
    休憩時間設定モデル
    """
    work_pattern_id = models.ForeignKey(WorkPattern, on_delete=models.CASCADE, verbose_name="勤務形態")
    ...
    def __str__(self):
        return f"{self.work_pattern_id.work_pattern_name} - {self.start_time} - {self.end_time}"
