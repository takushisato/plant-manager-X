...
class WorkRecord(BaseModel):
    ...
    work_pattern = models.ForeignKey(
        WorkPattern, on_delete=models.CASCADE, verbose_name="勤務形態"
    )
    ...
    def get_records_by_month(cls, month):
        ...
        return (
            cls.objects.filter(
                date__gte=start_date, date__lt=end_date, deleted_at__isnull=True
            )
    ...
    def get_records_by_user_and_month(cls, user, start_date, end_date):
        ...
        return cls.objects.filter(
            user=user, date__gte=start_date, date__lt=end_date, deleted_at__isnull=True
